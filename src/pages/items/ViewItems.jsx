...
const ViewItems = () => {
  ...
  const fetchPost = async () => {
    await getDocs(collection(firestore, "items")).then((querySnapshot) => {
  ...
  const deleteItem = async (event) => {
    ...
    try {
      await deleteDoc(
        doc(firestore, "items", items[event.currentTarget.id].id)
      );
      fetchPost();
      Swal.fire({
        position: "center",
    ...
    <div>
      ...
      <div className="flex flex-wrap justify-center">
        {items?.map((item, i) => (
          <div
            key={i}
            className="p-6 rounded-lg shadow-lg m-6 bg-white w-2/6 space-x-6 border border-gray-300"
          >
